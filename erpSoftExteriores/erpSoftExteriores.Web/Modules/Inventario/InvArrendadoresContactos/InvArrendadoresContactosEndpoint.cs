...
namespace erpSoftExteriores.Inventario.Endpoints
{
    ...
    using MyRepository = Repositories.InvArrendadoresContactosRepository;
    using MyRow = Entities.InvArrendadoresContactosRow;

    [RoutePrefix("Services/Inventario/InvArrendadoresContactos"), Route("{action}")]
    [ConnectionKey("Default"), ServiceAuthorize("AdministracionDeInventario")]
    public class InvArrendadoresContactosController : ServiceEndpoint
    {
        /******/
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            var columns = typeof(Columns.InvArrendadoresColumns);
            var name = "ContactosArrendadores_";
