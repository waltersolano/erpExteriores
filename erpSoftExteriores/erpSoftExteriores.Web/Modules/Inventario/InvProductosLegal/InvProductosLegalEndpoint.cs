...
namespace erpSoftExteriores.Inventario.Endpoints
{
    ...
    using MyRepository = Repositories.InvProductosLegalRepository;
    using MyRow = Entities.InvProductosLegalRow;

    [RoutePrefix("Services/Inventario/InvProductosLegal"), Route("{action}")]
    [ConnectionKey("Default"), ServiceAuthorize("Administration")]
    public class InvProductosLegalController : ServiceEndpoint
    {
        /******/
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            var columns = typeof(Columns.InvStatusColumns);
            var name = "Status_";
