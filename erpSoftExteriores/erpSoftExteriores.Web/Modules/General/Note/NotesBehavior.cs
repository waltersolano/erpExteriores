using erpSoftExteriores.Administration.Entities;
using erpSoftExteriores.General.Entities;
using erpSoftExteriores.General.Repositories;
using Serenity;
using Serenity.Data;
    ...
    public class NotesBehavior : BaseSaveDeleteBehavior, IImplicitBehavior, IRetrieveBehavior, IFieldBehavior
    {
        ...
        private void SaveNote(IUnitOfWork uow, NoteRow note, string entityType, Int64 entityId, Int64? noteId)
        {
            ...
            else
            {
                ...
                var checkUser = new NoteRepository().Retrieve(uow.Connection, new RetrieveRequest { EntityId = noteId });
                if (user.UserId == checkUser.Entity.InsertUserId)
                {
